...
export const useLocalStorage = (key, initialValue) => {
  ...
  const [storedValue, setStoredValue] = useState(() => {
    ...
    try {
      ...
      log.error(`useLocalStorage(${key}, ${initialValue}): will return initialValue because encountered error: ${error}`);
      return initialValue;
    }
  });
  ...
  const setValue = value => {
    try {
      ...
      log.error(`useLocalStorage(${key}, ${initialValue}): setValue(${value}): will return initialValue because encountered error: ${error} `);
    }
  };
