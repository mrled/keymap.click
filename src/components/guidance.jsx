...
export const GuideBar = () => {
  const { state, hydratedState, setGuide } = useContext(KeymapUiStateContext);

  const guideSelectBox = useRef();

  return (
    <>
      <div className="border border-gray-300 p-2 m-4 mx-12">
        <label htmlFor="guide-selector" className="p-2 m-2 w-1/3">
          Guide:
        </label>
        <select
          onChange={(event) => {
          ...
          name="Guides"
          id="guide-selector"
          className={classnames("p-2 m-2 w-1/3", {
          ...
          {Object.keys(hydratedState.keyMap.guides).map((guideName) => {
            return (
              <option key={guideName} value={guideName}>
                {hydratedState.keyMap.guides[guideName].fullName}
              </option>
            );
          })}
        </select>
