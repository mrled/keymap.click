...
/* An element with this class is identified as pysical key indicator
 */
...
const connectionPointTo = (element) => {
  ...
  // TO DO: clean this up, and change connectionPointFrom() to use a handler div like we do inside <Key>
...
export const getKeyConnections = () => {
  ...
  for (let indicator of renderedKeyIndicators) {
    ...
    targetKeyIds.forEach((targetKeyId) => {
      ...
      const targetCoords = connectionPointTo(targetKey);
      log.debug(
