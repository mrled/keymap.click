...
export const getKeyConnections = () => {
  ...
  const renderedKeyPointers = document.getElementsByClassName(
  ...
  for (let keyPointer of renderedKeyPointers) {
    ...
    const targetKeyIds = keyPointer.className
      .split(" ")
    ...
    targetKeyIds.forEach((targetKeyId) => {
      const targetCoords = connectionPointTo(
        document.getElementById(targetKeyId)
      );
      log.debug(
